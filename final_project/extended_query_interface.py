...
def print_menu():
	...
	print "11. View tweet count for teams with tweets"
	print "12. View tweet count for each team"
	print "13. View number of users tweeting for each team"
	print "14. View number of favorites teams have received"
	print "15. View number of favorites users received"
...
def tweet_count():
	...
	is_success = True
	query = "select t.team_name, t.team_ID, count(*) as tweet_count from Team t join Tweet s on t.Team_ID = s.team_ID group by t.team_name, t.team_ID order by tweet_count desc;"
	...
	try: 
		...
		is_success = False
		print "Tweet count query failed: " + e.strerror
...
def tweet_count_all():
	...
	is_success = True
	query = "select t.team_name, t.team_ID, count(s.tweet_id) as tweet_count from Team t left outer join Tweet s on t.team_ID = s.team_ID group by t.team_name, t.team_ID order by tweet_count desc;"
	...
	try: 
		...
		is_success = False
		print "Tweet Count all query failed: " + e.strerror
...
def user_count_all():
	...
	is_success = True
	query = "select t.team_name, t.team_ID, count(distinct s.screen_name) as user_count from Team t left outer join Tweet s on t.team_ID = s.team_ID group by t.team_name, t.team_ID order by user_count desc;"
	...
	try: 
		...
		is_success = False
		print "User Count all query failed: " + e.strerror

	return is_success


def favorites_per_team():
	
	
	is_success = True
	query = "select t.team_name, t.team_ID, count(s.favorite_count) as favorites from Team t left outer join Tweet s on t.team_ID = s.team_ID where favorite_count > 0 group by t.team_name, t.team_ID order by favorites desc;"
	...
	try: 
		...
		is_success = False
		print "Favorites per team query failed: " + e.strerror
...
def favorites_per_user():
	...
	is_success = True
	query = "select screen_name, favorite_count from tweet where favorite_count > 0 order by favorite_count desc;"
...
while True:		  
	...
	if choice==1:	 
		...
		print "You have chosen to see the tweet count of teams with tweets. Here are the results:"
		print"('Team','Team ID','Tweet Count')"
		tweet_count()

	elif choice== 12:	 
		print "You have chosen to see every teams tweet count. Here are the results:"
		print"('Team','Team ID','Tweet Count')"
		tweet_count_all()
	elif choice== 13:	 
		print "You have chosen to see the number of users tweeting about a team. Here are the results:"
		print"('Team','Team ID','Tweet Count')"
		user_count_all()

	elif choice== 14:	 
		print "You have chosen to see the number of favorites teams received. Here are the results:"
		print"('Team','Team ID','Favorites')"
		favorites_per_team()
	elif choice== 15:	 
		print "You have chosen to see the number of favorites users received. Here are the results:"
		print"('Team','Team ID','Favorites')"
		favorites_per_user()
